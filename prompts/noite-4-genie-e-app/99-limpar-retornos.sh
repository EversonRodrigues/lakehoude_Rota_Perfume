...
#
# Uso:  bash prompts/noite-4-genie-e-app/99-limpar-retornos.sh <profile> [--apagar]
#       Sem --apagar ele só MOSTRA o que apagaria.
#
# Para apagar a noite 4 inteira (app, Genie e tabela), use o 99-limpar-aula-04.
set -euo pipefail

PROFILE="${1:?uso: bash prompts/noite-4-genie-e-app/99-limpar-retornos.sh <profile> [--apagar]}"
CONFIRMA="${2:-}"
...
if [ "$CONFIRMA" != "--apagar" ]; then
  ...
  echo "Para apagar de verdade:  bash prompts/noite-4-genie-e-app/99-limpar-retornos.sh $PROFILE --apagar"
  exit 0
fi
