...
Abra o Catalog Explorer **antes de colar o prompt**. Não tem
nada lá. É esse o ponto de partida, e é o contraste que faz o resto da noite.

```bash
# 1. o catálogo da noite não existe
databricks catalogs list --profile <PERFIL> | grep rotaperfume || echo 'não existe'
...
- profile: <PERFIL>   (sempre passe --profile, nunca deixe implícito)
- host: <HOST_DO_WORKSPACE>
- SQL Warehouse: <WAREHOUSE_ID> (Serverless Starter Warehouse)
...
```bash
databricks fs ls dbfs:/Volumes/lakehouse_rotaperfume/bronze/raw/erp --profile <PERFIL>
```
...
bash scripts/subir-raw.sh <PERFIL>     # devolve o arquivo e rode de novo
```
